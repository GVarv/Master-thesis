...
```{r Densities of medians for different species, fig.height=15, fig.width=15}
colnames(med_diff.sp)=colnames(med_frac)
par(mfrow=c(5,4), mar=c(1,1,7,1))
...
mtext("Density of medians for number of different species in the species factories found after randomization", side = 3, line = -3, outer = TRUE, cex=2)
```
...
```
```{r Densities of medians fordifferent species/total fidings, fig.height=15, fig.width=20}
...
```{r Cumluative distribution of total findings, fig.height=15, fig.width=20}
par(mfrow=c(5,4), mar=c(4,1,7,1))
ecdf_total.sp=c()
for (i in 1:ncol(med_tot.sp)){
  ...
  plot(ecdf(med_diff.sp[,mn]),xlab = paste0("ECDF original species factories=",gg),ylab = "Distribution of x",main=paste0(mn), xlim=c(0,150))
  abline(v = medians_sp.fac["total.find_med" ,mn], col="red")
  legend("topright", legend=c("Original species factories median"), col="red", lwd=1)
...
```{r Cumluative distribution of different findings, fig.height=15, fig.width=20}
par(mfrow=c(5,4), mar=c(4,1,7,1))
for (i in 1:ncol(med_diff.sp)){
  mn=colnames(med_diff.sp)[i]
...
```{r Cumluative distribution of fraction, fig.height=15, fig.width=20}
par(mfrow=c(5,4), mar=c(4,1,7,1))
for (i in 1:ncol(med_frac)){
  mn=colnames(med_frac)[i]
  ...
  plot(ecdf(med_frac[,mn]),xlab = paste0("ECDF original species factories=",gg),ylab = "Distribution of x", xlim=c(0,1), main=paste0(mn))
  abline(v = medians_sp.fac["frac_med" ,mn], col="red")
  legend("topright", legend=c("Original species factories median"), col="red", lwd=1)
